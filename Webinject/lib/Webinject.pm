    ...
    for my $currentcasefile (@{$self->{'casefilelist'}}) { #process test case files named in config
        ...
        foreach (1 .. $repeat) {
            ...
            foreach (sort {$a<=>$b} keys %{$xmltestcases->{case}}) {  #process cases in sorted order
                ...
                if ($self->{'xnode'}) {  #if an XPath Node is defined, only process the single Node
                    $testnum = $self->{'xnode'};
                }
                ...
                if ($self->{'latency'} > $self->{'maxresponse'}) { $self->{'maxresponse'} = $self->{'latency'}; }  #set max response time
                if ($self->{'latency'} < $self->{'minresponse'}) { $self->{'minresponse'} = $self->{'latency'}; }  #set min response time
                $self->{'totalresponse'} = ($self->{'totalresponse'} + $self->{'latency'});  #keep total of response times for calculating avg
                $self->{'avgresponse'} = (int(1000 * ($self->{'totalresponse'} / $self->{'totalruncount'})) / 1000);  #avg response rounded to thousandths

                if($self->{'gui'}) { $self->{'gui'}->gui_updatemonstats(); }  #update timers and counts in monitor tab
                ...
                if ($self->{'case'}->{sleep}) {  #if a sleep value is set in the test case, sleep that amount
...
sub _finaltasks {
    ...
    #plugin modes
    if ($self->{'reporttype'}) {  #return value is set which corresponds to a monitoring program

        #Nagios plugin compatibility
        if ($self->{'reporttype'} eq 'nagios') { #report results in Nagios format
            my $end = defined $self->{'config'}->{globaltimeout} ? "$self->{'config'}->{globaltimeout};;0" : ";;0";
            ...
            elsif (($self->{'config'}->{globaltimeout}) && ($self->{'totalruntime'} > $self->{'config'}->{globaltimeout})) {
        ...
        elsif ($self->{'reporttype'} eq 'mrtg') { #report results in MRTG format
            if ($self->{'case'}->{'failedcount'} > 0) {
...
sub _whackoldfiles {
    ...
    if (-e $self->{'dirname'}."plot.plt")   { unlink $self->{'dirname'}."plot.plt"; }
    if (-e $self->{'dirname'}."plot.png")   { unlink $self->{'dirname'}."plot.png"; }
    if (glob($self->{'dirname'}."*.xml.tmp")) { unlink glob($self->{'dirname'}."*.xml.tmp"); }
